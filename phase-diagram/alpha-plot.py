...
OverlapMatrixn = np.zeros((ensembleCount, math.ceil(((m_f-1)/steps))))  # initiate overlap matrix
target = 0
...
for b in tqdm(range(0, ensembleCount)):
    ...
    for bit in list(rs):  # randomly pick up 25 percent and flip them
        u[bit] = (MemoryMatrix[target][bit] * -1)
...
pn = ".png"
if (type==2):
 filename = "_".join([str(type), str(n), str(k), str(p), suffix, pn])
